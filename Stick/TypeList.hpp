...
struct HasTypeHelper
{
    static constexpr bool value =
        std::is_same<typename L::Head, T>::value ? true : HasTypeHelper<typename L::Tail, T>::value;
};
...
struct TypeAtHelper
{
    ...
    using Type = typename std::conditional<
        Comperator::value,
        typename Comperator::Head,
        typename TypeAtHelper<typename Comperator::Tail, IDX>::Type>::type;
};
...
{
    using List =
        TypeList<typename L::Head, typename AppendTypeListHelper<typename L::Tail, H, T>::List>;
};
...
{
    using List =
        typename detail::AppendTypeListHelper<L, typename L2::Head, typename L2::Tail>::List;
};
