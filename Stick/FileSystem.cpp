...
namespace stick
{
    namespace fs
    {
        ...
        FileStatus::FileStatus(const String & _path):
            m_bIsValid(true)
        {
#ifdef STICK_PLATFORM_POSIX
        ...
        DirectoryEntry::DirectoryEntry(const String & _path):
            m_path(_path),
            m_status(_path)
        {
        ...
        DirectoryIterator::DirectoryIterator(const String & _path):
            m_dir(nullptr),
            m_path(_path)
        {
        ...
        Error createSymbolicLink(const String & _path1, const String & _path2)
        {
            ...
            if (result == -1)
                Error(ec::SystemErrorCode(errno), "Could not create symbolic link.", STICK_FILE, STICK_LINE);
            return Error();
        }
        ...
        Result<String> readSymbolicLink(const String & _path)
        {
            ...
            else
            {
                ...
                //readlink does not guarantee null-termination, so we manually make sure that the string is terminated.
                p[actualLen] = '\0';
