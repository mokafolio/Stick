...
inline Error validateName(const String & _name)
{
    if (!_name.length())
        return Error(
            ec::InvalidArgument, "The argument name can't be empty", STICK_FILE, STICK_LINE);

    if (_name.length() < 2 && (_name[0] != '-' || _name.length() == 3))
        return Error(ec::InvalidArgument,
                     "Short argument names must start with \"-\"",
                     STICK_FILE,
                     STICK_LINE);

    if (_name.length() > 2 && (_name[0] != '-' || _name[1] != '-'))
        return Error(ec::InvalidArgument,
                     "Long argument names must start with \"--\"",
                     STICK_FILE,
                     STICK_LINE);
...
ArgumentParser::Argument::Argument() : bOptional(true), argCount(0), bArgumentWasProvided(false)
{
}

ArgumentParser::Argument::Argument(const String & _shortName,
                                   const String & _name,
                                   Int32 _count,
                                   bool _bOptional,
                                   const String & _info) :
...
ArgumentParser::ArgumentParser(const String & _info) : m_info(_info), m_requiredCount(0)
{
...
Error ArgumentParser::addArgument(const String & _shortName,
                                  const String & _name,
                                  UInt8 _argCount,
                                  bool _bOptional)
{
...
Error ArgumentParser::addArgumentHelper(const Argument & _arg)
{
    ...
    {
        return Error(ec::InvalidArgument,
                     "An Argument needs either a short name and/or a long name.",
                     STICK_FILE,
                     STICK_LINE);
    }
...
Error ArgumentParser::parse(const char ** _args, UInt32 _argc)
{
    ...
    for (Size i = 1; i < _argc; ++i)
    {
        ...
        if (it != m_indices.end())
        {
            ...
            if (active)
            {
                if (activeArgTargetCount == '+' && currentCount < 1)
                {
                    String err;
                    err.appendFormatted(
                        "Expected at least 1 argument for %s, got %i.", activeName, currentCount);
                    return Error(ec::InvalidArgument, err, STICK_FILE, STICK_LINE);
                }
                else if ((activeArgTargetCount != '*' && activeArgTargetCount != '+' &&
                          currentCount < activeArgTargetCount) ||
                         (activeArgTargetCount == '+' && currentCount < 1))
                {
                    String err;
                    err.appendFormatted("Expected %i arguments for %s, got %i.",
                                        activeArgTargetCount,
                                        activeName,
                                        currentCount);
                    return Error(ec::InvalidArgument, err, STICK_FILE, STICK_LINE);
                }
            ...
            if ((activeArgTargetCount == '+' && left < 1) ||
                (activeArgTargetCount != '*' && activeArgTargetCount != '+' &&
                 left < activeArgTargetCount))
            {
                String err;
                err.appendFormatted(
                    "Expected %i arguments for %s, there are only %i left to parse.",
                    tc,
                    activeName,
                    left);
                return Error(ec::InvalidArgument, err, STICK_FILE, STICK_LINE);
        ...
        {
            if (activeArgTargetCount != '*' && activeArgTargetCount != '+' &&
                currentCount > activeArgTargetCount)
            {
                String err;
                err.appendFormatted("Expected %i arguments for %s, got %i.",
    ...
    for (auto & arg : m_args)
    {
        if (!arg.bOptional && arg.argCount != '*' && arg.argCount > 0)
        {
            if (!arg.values.count())
                return Error(
                    ec::InvalidArgument,
                    String::concat("Required argument ", arg.printableIdentifier(), " is missing."),
                    STICK_FILE,
                    STICK_LINE);
        }
    }
...
String ArgumentParser::usage() const
{
    ...
    {
        ret.append(
            AppendVariadicFlag(), "[", arg.identifier(), detail::argumentSignature(arg), "] ");
    }
    ret.append("\n");
    return ret;
}
