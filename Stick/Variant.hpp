...
struct StaticMax<arg1, arg2, others...>
{
    static constexpr Size value =
        arg1 >= arg2 ? StaticMax<arg1, others...>::value : StaticMax<arg2, others...>::value;
};
...
struct DirectType<T, First, Ts...>
{
    static constexpr TypeID typeID =
        std::is_same<T, First>::value ? TypeInfoT<T>::typeID() : DirectType<T, Ts...>::typeID;
};
...
{
    using type = typename std::conditional<std::is_convertible<T, First>::value,
                                           First,
                                           typename ConvertibleType<T, Ts...>::type>::type;
    //@TODO: Add check to make sure that T can only convert to one type (to catch ambigous
    //conversions)
    static constexpr TypeID typeID = std::is_convertible<T, First>::value
                                         ? TypeInfoT<First>::typeID()
                                         : ConvertibleType<T, Ts...>::typeID;
};
...
struct Traits
{
    ...
    static constexpr TypeID typeID = bIsDirect ? DirectType<ValueType, Ts...>::typeID
                                               : ConvertibleType<ValueType, Ts...>::typeID;
    static constexpr bool bIsValid = typeID != 0;
    using TargetType = typename std::
        conditional<bIsDirect, ValueType, typename ConvertibleType<ValueType, Ts...>::type>::type;
};
...
// {
//     //@TODO: remove volatile, too?
//     using ValueType = typename std::remove_reference<typename std::remove_const<T
//     &>::type>::type; static constexpr bool bIsDirect = DirectType<T &, Ts...>::typeID != 0;
//     static constexpr TypeID typeID = bIsDirect ? DirectType<T &, Ts...>::typeID :
//     ConvertibleType<T &, Ts...>::typeID; static constexpr bool bIsValid = typeID != 0;
//     //@TODO: ConvertibleType should most likely differ between ref/no ref aswell, so that
//     implicit conversions
//     //between references can become possible with the correct storage.
//     using TargetType = typename std::conditional<bIsDirect, std::reference_wrapper<ValueType>,
//     typename ConvertibleType<T &, Ts...>::type>::type;
// };
...
class Variant
{
  ...
  public:
    ...
    inline Variant() : m_typeID(0)
    {
    }

    inline Variant(const Variant & _other) : m_typeID(_other.m_typeID)
    {
        Helper::copy(_other.m_typeID, &_other.m_storage, &m_storage);
    }

    inline Variant(Variant && _other) : m_typeID(_other.m_typeID)
    {
    ...
    inline typename detail::ReturnTypeTraits<T>::ConstReferenceType get() const
    {
        ...
        // detail::ReturnTypeTraits<T>::ValueType*>(&m_storage);
    }

    template <class T>
    inline typename detail::ReturnTypeTraits<T>::ReferenceType get()
    {
        return const_cast<typename detail::ReturnTypeTraits<T>::ReferenceType>(
            const_cast<const Variant *>(this)->get<T>());
    }
