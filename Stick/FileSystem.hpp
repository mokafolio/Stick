 ...
 */
class STICK_API DirectoryIterator : public std::iterator<std::input_iterator_tag, DirectoryEntry>
{
  public:
    /**
    ...
    : public std::iterator<std::input_iterator_tag, DirectoryEntry>
{
  ...
  private:
    struct Node
    {
        Node(const String & _path, Node * _parent) : m_parent(_parent), m_it(_path)
        {
        }
    ...
    Node * m_node;
    bool m_bGotSubDirectory; // flag that indicates that a new node should be created on the next
                             // iteration
};
 ...
 * @param _path The path of the directory.
 */
STICK_API Result<DirectoryEntryArray> listDirectory(const String & _path,
