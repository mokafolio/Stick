...
class String
{
  ...
  public:
    ...
    {
        if (isEmpty() && (!_str || (_str && strlen(_str) == 0)))
            return true;
        else if (isEmpty() || !_str)
            return false;
    ...
    inline bool operator<=(const String & _str) const
    {
        if (!m_cStr && !_str.m_cStr)
            return true;
        if (!_str.m_cStr || !m_cStr)
            return false;
        return strcmp(m_cStr, _str.m_cStr) < 0;
    }
    ...
    {
        return String(begin() + _pos,
                      _length == InvalidIndex ? begin() + m_length : begin() + _pos + _length,
                      _alloc);
    }
    ...
    inline String & insert(Size _idx,
                           const String & _other,
                           Size _otherIndex,
                           Size _otherLen = InvalidIndex)
    {
    ...
    inline String toUpper() const
    {
        String ret(*this);
        std::transform(
            ret.begin(), ret.end(), ret.begin(), [](UInt8 _c) { return std::toupper(_c); });
        return ret;
    }
    ...
    inline String toLower() const
    {
        String ret(*this);
        std::transform(
            ret.begin(), ret.end(), ret.begin(), [](UInt8 _c) { return std::tolower(_c); });
        return ret;
    }
    ...
    inline static String toHexString(UInt64 _i,
                                     UInt32 _width = 0,
                                     bool _bUpperCase = true,
                                     bool _bShowBase = false,
    ...
    inline static String toHexString(UInt32 _i,
                                     UInt32 _width = 0,
                                     bool _bUpperCase = true,
                                     bool _bShowBase = false,
                                     Allocator & _alloc = defaultAllocator())
    {
        return String::toHexString(
            static_cast<UInt64>(_i), _width, _bUpperCase, _bShowBase, _alloc);
    }

    inline static String toHexString(Int64 _i,
                                     UInt32 _width = 0,
                                     bool _bUpperCase = true,
                                     bool _bShowBase = false,
                                     Allocator & _alloc = defaultAllocator())
    {
        ...
        {
            // TODO: this could be easily made more efficient as this will still be two memory
            // allocations
            return String::concat(
                "-",
                toHexString(
                    static_cast<UInt64>(labs(_i)), _width, _bUpperCase, _bShowBase, _alloc));
        }
    }

    inline static String toHexString(Int32 _i,
                                     UInt32 _width = 0,
                                     bool _bUpperCase = true,
                                     bool _bShowBase = false,
                                     Allocator & _alloc = defaultAllocator())
